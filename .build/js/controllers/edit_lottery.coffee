...
define ['console', 'humane', 'moment'], (console, humane, moment)->
  return ($scope, $http, data, $modalInstance)->
    ...
    $scope.$watch 'channel', ->
      if data._id
        $scope.link_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx1f9fe13fd3655a8d&redirect_uri=http://lottery.rsct.com/init_auto&state=c___'+$scope.channel+';;p___lottery;;id___'+data._id+'&response_type=code&scope=snsapi_base&connect_redirect=1#wechat_redirect'
